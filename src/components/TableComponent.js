...
export const SaveBtn = ({ data }) => {
  const { saveCoins, removeCoins, coins } = useContext(StorageContext);
  ...
  return (
    <button
      className="border-0 ml-11 outline-0 bg-none cursor-pointer"
      onClick={(e) => {
        handleClick(e, data);
      }}
    >
      <svg
        className={`w-[1.5rem] hover:fill-cyan ${
          coins.includes(data.id) ? "fill-cyan" : "fill-gray-100"
        }`}
        width="30"
        height="30"
        viewBox="0 0 30 30"
    ...
    <div>
      <div className="mt-9 flex flex-col border border-gray-100">
        {cryptoData ? (
          ...
          <div className="w-full g-full flex justify-center items-center">
            <div
